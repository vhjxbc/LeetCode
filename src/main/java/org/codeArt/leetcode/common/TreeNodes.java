package org.codeArt.leetcode.common;
...
public class TreeNodes {
    ...
    public static TreeNode createTreeNode(Integer[] values) {
        ...
        if (i < values.length) {
            Integer element = values[i];  //拿到先根遍历序列的对应元素
            i++;  //i自增1
            if (element != null) {  //不能 element != "^",因为 T 不一定是 String
                p = new TreeNode(element);  //创建叶子结点
                p.left = createTreeNode(values);  //创建 p 的左子树,递归调用
                p.right = createTreeNode(values);  //创建 p 的右子树,递归调用
            }
        }
